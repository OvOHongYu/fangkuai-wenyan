...
- **拼音字母索引**：卡片库（实词 / 虚词 / 对比 / 收藏）按标题拼音首字母分组排序，右侧 A–Z 索引栏支持点按跳转与按住拖动快速翻找（多音字取数据中标注的首选读音）

### 二、释义 ↔ 篇目双向定位（本项目的差异化功能）

- **篇目库**：覆盖统编版高中语文**必修上/下册、选择性必修上/中/下册共 61 篇**文言诗文，每篇含正文、全文翻译与**按义项标注的字词注释（2563 条）**
- **按义项查篇目**：卡片中每个义项旁的「出现篇目 ▸」列出该义在该篇中出现的全部篇目。例如「而 = 转折」可查得 28 篇、「以 = 因为」18 篇、「爱 = 爱护」6 篇
- **3/4 屏定位阅读面板**：点击篇目名后自底部滑入，**自动滚动到该字首次出现处**，并把正文中这个字的每一次出现都高亮：
  - 🟩 **与当前义项相同** → 一色
  - 🟧 **同一个字的其他义项** → 另一色
...
│  └─ util/                               # 遗忘曲线、出题器、设置存储
├─ assets/
│  ├─ cards/                              # shici_a~h.json(129) / xuci*.json(18) / compare.json(3)
